...
async function run() {
  ...
  if (user) {
    ...
    const { data: { user: updatedUser }, error: updateError } = await supabase.auth.admin.updateUserById(user.id, { user_metadata: { username, role }, email_confirm: true });
    if (updateError) { console.error(updateError.message); process.exit(1); }
    userId = user.id;
    console.log('User updated successfully.');
  } else {
    console.log('Creating new user...');
    const { data: { user: newUser }, error: createError } = await supabase.auth.admin.createUser({ email, password, user_metadata: { username, role }, email_confirm: true });
    if (createError) { console.error(createError.message); process.exit(1); }
    userId = newUser.id;
    console.log('User created successfully.');
  }
  const { error: upsertError } = await supabase.from('profiles').upsert({ id: userId, username, role });
  if (upsertError) { console.error(upsertError.message); process.exit(1); }
